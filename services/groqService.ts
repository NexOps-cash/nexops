...
export const generateSmartContract = async (prompt: string): Promise<GenerationResponse> => {
  ...
  3. DO NOT return the code as a single long line.
  4. Ensure the code is valid CashScript (.cash).
...
export const chatWithAssistant = async (
  ...
  RESPONSE SCHEMA:
  {
    "response": "A markdown string explaining what you did",
    "fileUpdates": [
...
export const fixSmartContract = async (code: string, instructions: string): Promise<GenerationResponse> => {
    ...
    2. Maintain the original logic where possible.
    3. Add comments explaining the security fixes.
    4. RETURN FORMATTED CODE with proper newlines and indentation. Do not minify.
    
    Return JSON with the fixed code and a summary of changes.
...
export const auditSmartContract = async (code: string): Promise<AuditReport> => {
