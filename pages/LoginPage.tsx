...
export const LoginPage: React.FC = () => {
  ...
  const { user, isLoading, signInWithGithub, authEstablishError, clearAuthEstablishError } = useAuth();
  ...
  const handleGithub = async () => {
    const stored = sessionStorage.getItem(AUTH_RETURN_KEY);
    sessionStorage.removeItem(AUTH_RETURN_KEY);
    const candidate = resolveReturnCandidate(stored, queryReturn);
    setOAuthPendingReturn(candidate);
  ...
  return (
    <div className="h-full min-h-[50vh] w-full flex items-center justify-center bg-nexus-900 text-white">
      <div className="text-center space-y-6 max-w-sm p-10 bg-white/5 rounded-3xl border border-white/10 shadow-2xl backdrop-blur">
        ...
        <div className="space-y-2">
          ...
          <p className="text-white/40 text-sm leading-relaxed">Continue with GitHub to access NexOps.</p>
        </div>
        {authEstablishError && (
          <div className="rounded-lg border border-red-500/40 bg-red-500/10 p-3 text-sm text-red-200 space-y-2">
            <p>{authEstablishError}</p>
            <button type="button" onClick={() => clearAuthEstablishError()} className="text-xs underline">
              Dismiss
            </button>
          </div>
        )}
